...
encoder = ResNet(BasicBlock,[2,2,2,2],im_grayscale=im_grayscale,im_gradients=im_gradient)
...
if False:
	data = np.load("./data/cal256data.npy")
	labels = np.load("./data/cal256labels.npy")

	i_r_idx = np.random.permutation(len(data))
	data = data[i_r_idx]
	if im_grayscale:
		data = np.expand_dims(data.sum(axis=1),1)/3
	labels = labels[i_r_idx]
	...
	test_labels = labels[train_len+1:]


train_loader = torchvision.datasets.CIFAR100(os.getcwd(),download=True,transform=image_transforms())
train_data = train_loader.train_data
train_labels = train_loader.train_labels
train_len = len(train_data)

test_loader = torchvision.datasets.CIFAR100(os.getcwd(),download=True,train=False,transform=image_transforms())
...
def train():
	...
	for epoch in range(epochs):
		r_idx = np.random.permutation(train_data.shape[0])
		...
		for i in tqdm(range(0,train_len,batch_size)):
			...
			if train_decoder:
				...
				if i % 100 == 0:
					idx_step = int(epoch*train_len/batch_size/train_decoder_period)
